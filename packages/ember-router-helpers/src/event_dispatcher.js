...
if (macroCondition(dependencySatisfies('ember-href-to', '*'))) {
  throw new Error(
    '`ember-href-to` add-on detected, please migrate from `{{href-to}}` to `{{url-for}}` helper.'
  );
}
...
export default class HTMLAnchorEventDispatcher extends Ember.EventDispatcher {
  ...
  recognizeAnchorURL(event, element) {
    ...
    if (
      view &&
      typeof view.toString === 'function' &&
      view.toString() === '@ember/routing/link-component'
    ) {
      return null;
    }
